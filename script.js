...
const azkarData = {
    ...
    ruqyah:[
        {text:"بِسْمِ اللَّهِ أَرْقِيكَ، مِنْ كُلِّ شَيْءٍ يُؤْذِيكَ، مِنْ شَرِّ كُلِّ نَفْسٍ أَوْ عَيْنِ حَاسِدٍ، اللَّهُ يَشْفِيكَ، بِسْمِ اللَّهِ أَرْقِيكَ.",count:3,proof:"الدليل: رقية جبريل للنبي ﷺ (صحيح مسلم)."},{text:"أَعُوذُ بِكَلِمَاتِ اللَّهِ التَّامَّةِ، مِنْ كُلِّ شَيْطَانٍ وَهَامَّةٍ، وَمِنْ كُلِّ عَيْنٍ لَامَّةٍ.",count:3,proof:"الدليل: صحيح البخاري."},{text:"بِسْمِ اللَّهِ (ثلاثاً)\nأَعُوذُ بِاللَّهِ وَقُدْرَتِهِ مِنْ شَرِّ مَا أَجِدُ وَأُحَاذِرُ (سبعاً).",count:1,proof:"الدليل: توضع اليد على موضع الألم. (صحيح مسلم)."},{text:ayatAlKursi,count:1,proof:"آية الكرسي من أعظم الرقية."},{text:surahAlIkhlas,count:3,proof:"سورة الإخلاص."},{text:surahAlFalaq,count:3,proof:"سورة الفلق."},{text:surahAnNas,count:3,proof:"سورة الناس."}
    ]
};
...
window.hideSplash = () => { 
    document.getElementById('splash-screen').classList.replace('active','hidden'); 
    document.getElementById('home-screen').classList.replace('hidden','active'); 
};
window.goHome = () => { 
    if(quranAudio) stopAudio(); 
    document.querySelectorAll('.screen').forEach(s => {
        if(s.classList.contains('active')) s.classList.replace('active','hidden');
    }); 
    document.getElementById('home-screen').classList.replace('hidden','active'); 
};
window.toggleTheme = () => { 
...
// نظام الأذكار
window.openAzkarMenu = () => { 
    goHome(); 
    document.getElementById('home-screen').classList.replace('active','hidden'); 
    document.getElementById('azkar-categories-screen').classList.replace('hidden','active'); 
};
window.startAzkar = (type) => { 
    currentCategory = azkarData[type]; currentIndex = 0; 
    document.getElementById('azkar-categories-screen').classList.replace('active','hidden'); 
    document.getElementById('azkar-screen').classList.replace('hidden','active'); 
    loadZikr(); 
};
function loadZikr() {
    if(currentIndex >= currentCategory.length) return openAzkarMenu();
...
// السبحة والقبلة والمواقيت
window.openMasbaha = () => { goHome(); document.getElementById('home-screen').classList.replace('active','hidden'); document.getElementById('masbaha-screen').classList.replace('hidden','active'); };
...
window.openQibla = () => { 
    goHome(); 
    document.getElementById('home-screen').classList.replace('active','hidden'); 
    document.getElementById('qibla-screen').classList.replace('hidden','active'); 
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
            ...
            window.addEventListener('deviceorientationabsolute', (e) => {
                ...
                if (compass) {
                    ...
                    else { st.innerText="قم بلف الهاتف"; st.style.color="var(--accent-color)"; }
                }
            }, true);
        });
    }
...
window.openPrayerTimes = () => { goHome(); document.getElementById('home-screen').classList.replace('active','hidden'); document.getElementById('prayer-times-screen').classList.replace('hidden','active'); calculatePrayerTimes(); };
    ...
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (pos) => {
            ...
            try {
                ...
                document.getElementById('prayer-location').innerText = `📍 ${geoData.city || geoData.locality || 'موقعك'}`;
...
// القرآن المتزامن
let surahListCached=[], currentSurahNumber=null, currentAyahsAudioUrls=[], currentPlayingAyahIndex=-1, isPlayingQuran=false;
const quranAudio = document.getElementById('quran-audio');
window.openQuranIndex = async () => { 
    goHome(); document.getElementById('home-screen').classList.replace('active','hidden'); document.getElementById('quran-index-screen').classList.replace('hidden','active');
    if(surahListCached.length===0) {
        ...
        try{
            ...
            surahListCached.forEach(s => {
                ...
                b.onclick = ()=>loadSurah(s.number, s.name); l.appendChild(b);
            });
        }catch(e){}
    }
};
    ...
    try {
        ...
        data.reciters.forEach(r => { if(r.moshaf && r.moshaf.length>0) {
            r.moshaf.forEach(m => {
                ...
                allRecitersCached.push({name: name, server: m.server});
                let opt = document.createElement('option'); opt.value = m.server; opt.innerText = name;
                if(m.server.includes('afs')) opt.selected=true; 
                sel.appendChild(opt);
            });
        }});
    } catch(e){}
...
window.loadSurah = async (number, name) => {
    document.getElementById('quran-index-screen').classList.replace('active','hidden'); document.getElementById('quran-reader-screen').classList.replace('hidden','active');
    ...
    try {
        // نأتي بالنص فقط، والصوت سنركبه يدويا لتجنب بطء الـ API المزدوج
...
window.playAyah = (index) => {
    let textDiv = document.getElementById('quran-text');
    let totalAyahs = textDiv.getElementsByClassName('ayah-span').length;
    if(index >= totalAyahs) { stopAudio(); return; }
    ...
    // رقم السورة 3 أرقام، ورقم الآية 3 أرقام
    let surahPad = currentSurahNumber.toString().padStart(3,'0');
    let ayahPad = (index + 1).toString().padStart(3,'0');
    
    quranAudio.src = `${server}${surahPad}${ayahPad}.mp3`;
    quranAudio.play();
    isPlayingQuran = true; document.getElementById('play-pause-btn').innerText="⏸ إيقاف";
};

quranAudio.onended = () => { playAyah(currentPlayingAyahIndex + 1); };

window.togglePlayPause = () => {
    if(isPlayingQuran) { quranAudio.pause(); isPlayingQuran=false; document.getElementById('play-pause-btn').innerText="▶ تشغيل"; }
    else { 
        if(currentPlayingAyahIndex === -1) playAyah(0); 
        else { quranAudio.play(); isPlayingQuran=true; document.getElementById('play-pause-btn').innerText="⏸ إيقاف"; }
    }
};
window.stopAudio = () => { quranAudio.pause(); isPlayingQuran=false; document.getElementById('play-pause-btn').innerText="▶ تشغيل"; if(currentPlayingAyahIndex !== -1 && document.getElementById(`ayah-${currentPlayingAyahIndex}`)) document.getElementById(`ayah-${currentPlayingAyahIndex}`).classList.remove('ayah-active'); currentPlayingAyahIndex = -1; };
window.changeReciterAndPlay = () => { if(currentSurahNumber && isPlayingQuran && currentPlayingAyahIndex !== -1) playAyah(currentPlayingAyahIndex); };
window.goBackToQuranIndex = () => { stopAudio(); document.getElementById('quran-reader-screen').classList.replace('active','hidden'); document.getElementById('quran-index-screen').classList.replace('hidden','active'); };
